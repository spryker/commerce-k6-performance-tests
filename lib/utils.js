...
export function getSeries(targetVus, stageDuration, startFrom = 1) {
    const result = [];

    while (startFrom <= targetVus) {
        let delta = startFrom < 10 ? 1 : 10;
        const targetValue = startFrom + delta <= targetVus ? startFrom + delta : targetVus;
        result.push({ duration: `${stageDuration}m`, target: targetValue });
        startFrom += delta;
    }
