...
export class CartHelper {
  ...
  getParamsWithAuthorization(
    email = this.customerHelper.getDefaultCustomerEmail(),
  ...
  deleteCarts(carts, params) {
    if (carts.data) {
      ...
      carts.data.forEach(function (cart) {
        let deleteCartResponse = self.http.sendDeleteRequest(
          self.http.url`${self.getCartsUrl()}/${cart.id}`,
          null,
          params,
          false
        );
  ...
  deleteCart(customerEmail, cartId, thresholdTag = null) {
    ...
    const response = this.http.sendDeleteRequest(
