...
export class SharedBrowserCheckoutScenario extends AbstractScenario {
  ...
  productPage() {
    //product page
    const productPageResponse = this.http.sendGetRequest(
      this.http.url`${this.getStorefrontBaseUrl()}/en/${'stapelstuhl-mit-geschlossenem-ruecken-M83'}`
    );
  ...
  checkoutPage() {
    const checkoutResponse = this.http.sendGetRequest(this.http.url`${this.getStorefrontBaseUrl()}/en/checkout`, {
      redirects: 0,
    });
  ...
  addressPage() {
    //address
    const addressStepResponse = this.http.sendGetRequest(
      this.http.url`${this.getStorefrontBaseUrl()}/en/checkout/address`
    );
  ...
  shipmentPage() {
    const shipmentStepResponse = this.http.sendGetRequest(
      this.http.url`${this.getStorefrontBaseUrl()}/en/checkout/shipment`
    );
  ...
  paymentPage() {
    const paymentStepResponse = this.http.sendGetRequest(
      this.http.url`${this.getStorefrontBaseUrl()}/en/checkout/payment`
    );
  ...
  summaryPage() {
    const summaryStepResponse = this.http.sendGetRequest(
      this.http.url`${this.getStorefrontBaseUrl()}/en/checkout/summary`
    );
    ...
    const successPageResponse = this.http.sendGetRequest(
      this.http.url`${this.getStorefrontBaseUrl()}/en/checkout/success`
    );
