...
export class ApiGetScenario extends AbstractScenario {
  ...
  execute() {
    ...
    group(self.group, function () {
      ...
      let responseProducts = self.http.sendGetRequest(
        self.http
          .url`${self.getBackendApiUrl()}/dynamic-entity/product-abstracts?include=productAbstractStores,productAbstractProducts,productRelations,productAbstractPriceProducts,productAbstractCategories,productAbstractLocalizedAttributes,productLabelProductAbstracts,productImageSets&page[offset]=0&page[limit]=100`,
        requestParams,
        false
      );
