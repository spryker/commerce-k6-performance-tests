...
export class ApiPatchPayloadScenario extends ApiPostPayloadScenario {
  ...
  execute(productTemplate, productConcreteTemplate, productLabelTemplate) {
    ...
    group(self.group, function () {
      ...
      if (self.responseProducts === null) {
        ...
        self.responseProducts = self.createProductsWithLabels(
          requestParams,
          productTemplate,
      ...
      do {
        ...
        if (updateResult.status !== 200) {
          ...
          console.warn(
            `Start sleeping because of request for products ${self.type} failed. Response status: ${updateResult.status}. Amount Of Tries: ${count}, timeout: ${sleepingInterval} sec. thread:${getThread()}, iteration: ${getIteration()}`
          );
        ...
        if (count > self.retryLimit) {
          console.error(
            `Request for products ${self.type}: was not able to process request within ${count} tries, thread:${getThread()}, iteration: ${getIteration()}`
          );
          break;
        }
  ...
  updateProducts(responseProducts, requestParams) {
    ...
    let updateResult = this.http.sendPatchRequest(
