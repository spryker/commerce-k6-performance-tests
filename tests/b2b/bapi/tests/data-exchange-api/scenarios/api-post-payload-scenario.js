...
export class ApiPostPayloadScenario extends AbstractScenario {
  ...
  execute(productTemplate, productConcreteTemplate, productImageTemplate, productLabelTemplate) {
    ...
    group(self.group, function () {
      ...
      let responseProducts = self.createProductsWithLabels(
        requestParams,
        productTemplate,
        productConcreteTemplate,
      ...
      do {
        ...
        if (productImageResponse.status !== 201) {
          ...
          console.warn(
            `Start sleeping because of request for images creation failed. Response status: ${productImageResponse.status}. Amount Of Retries: ${count}, timeout: ${sleepingInterval} sec`
          );
  ...
  createProductsWithLabels(requestParams, productTemplate, productConcreteTemplate, productLabelTemplate) {
    ...
    do {
      ...
      if (responseProducts.status !== 201) {
        ...
        console.warn(
          `Start sleeping because of request for products ${this.type} failed. Response status: ${responseProducts.status}. Amount Of Retries: ${count}, timeout: ${sleepingInterval} sec. thread:${getThread()}, iteration: ${getIteration()}`
        );
        sleep(sleepingInterval);
      ...
      if (count > this.retryLimit) {
        console.error(
          `Request for products ${this.type}: was not able to process request within ${count} iterations. thread:${getThread()}, iteration: ${getIteration()}`
        );
        break;
      }
  ...
  getLabels(productLabelTemplate, requestParams) {
    if (this.productLabelId === null) {
      ...
      let productLabelIds = this.http.sendPostRequest(
        this.http.url`${this.getBackendApiUrl()}/dynamic-entity/product-labels`,
        payloadLabels,
        requestParams,
        false
      );
  ...
  createProducts(productTemplate, productConcreteTemplate, productLabelId, requestParams) {
    ...
    let payloadProducts = this.payloadGenerator.generateProducts(
      productTemplate,
      productConcreteTemplate,
      productLabelId
    );
    let responseProducts = this.http.sendPostRequest(
  ...
  createProductsImages(productImageTemplate, responseProducts, requestParams) {
    ...
    let responseImageSet = this.http.sendPostRequest(
      this.http.url`${this.getBackendApiUrl()}/dynamic-entity/product-images`,
      payloadImageSet,
      requestParams,
      false
    );
