...
export class CartScenario extends SharedCartScenario {
  _setUp(requestParams) {
    ...
    const cartResponse = this.http.sendGetRequest(
      this.http.url`${this.cartHelper.getCartsUrl()}/${cart.id}/?include=items`,
      requestParams,
      false
    );
    ...
    if (cartRelationships) {
      ...
      items.data.forEach(function (item) {
        let cartItemDeleteResponse = self.http.sendDeleteRequest(
          self.http.url`${self.cartHelper.getCartsUrl()}/${cart.id}/items/${item.id}`,
          null,
          requestParams,
          false
        );
